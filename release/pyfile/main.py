...
from UI.mainWidget import Ui_MainWindow
from UI.addEditCoffeeForm import Ui_AddEditForm
import sys
...
class AddEditForm(QMainWindow, Ui_AddEditForm):
    ...
    def operate(self):
        try:
            ...
            if self.operation == 'INSERT':
                cur.execute("""INSERT or REPLACE INTO coffee VALUES(?, ?, ?, ?, ?, ?, ?)""",
                            (self.parent().coffee.rowCount() + 1, self.variety.currentIndex() + 1,
                             self.roast_degree.currentIndex() + 1,
...
class Capuccino(QMainWindow, Ui_MainWindow):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
